...
export const Pool = forwardRef(
  (
    ...
    const lanes = useMemo(
      () => newCellData.lanes.map((lane, index) => getLane(lane, index)),
      [newCellData.lanes, getLane]
    );
      ...
      <>
        ...
        <Row ref={ref} className={"lanes"}>
          {direction === "horizontal" ? (
            ...
            <FormGroup
              required={!!newCellData.required}
              warning={newCellData.warning}
              warnable={newCellData.warnable}
              element={
                <>
                  {!isDesigner && (
                    <InstanceListHeaderItem span={24}>
                      {newCellData.lanes[0].cellDataList.map((item) => (
                        <div key={item.id}>
                          {item.required && (
